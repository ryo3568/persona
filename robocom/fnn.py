...
def load_data(test_vid, profile_vid):
    ...
    _, _, _, _, TS, _, _, _, Text, _, _, vid = pickle.load(open(path, 'rb'), encoding='utf-8')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pmode', type=int, default=0) # 0: ベースライン, 1: 性別, 2: 年齢(2クラス), 3: 年齢(3クラス), 4: 年齢(6クラス) 5: 性別 & 年齢(2クラス), 6: 性別 & 年齢(3クラス), 7: 性別 & 年齢(6クラス)
    ...
    path = f'../data/Hazumi_features/Hazumiall_features.pkl'
    _, _, _, _, _, _, _, _, _, _, _, vid = pickle.load(open(path, 'rb'), encoding='utf-8')
    ...
    for profile_n in range(profile_num):
        ...
        profile_vid = []
        for id in vid:
            user_profile = profiling(args.pmode, id)
        ...
        '''
        if args.balanced:
            torch.save(model.state_dict(), f'results/binary/fnn/model-balanced-{args.pmode}-{profile_n}.pth')
        else:
            # torch.save(model.state_dict(), f'results/binary/fnn/model-{args.profile}-{profile_n}.pth')
            torch.save(model.state_dict(), f'results/cpu-model-{args.pmode}-{profile_n}.pth')
