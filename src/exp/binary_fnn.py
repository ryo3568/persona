...
def load_data(testuser, modal, profile):
    path = f'../../data/Hazumi_features/Hazumi1911_features_binary.pkl'
...
if __name__ == '__main__':
    ...
    for test_user in tqdm(users):
        ...
        print('test user: {}, acc: {:.3f}, f1: {:.3f}, auc: {:.3f}'.format(test_user, acc, f1, auc_score))

        results[test_user] = {
            'acc': float(round(acc, 3)),
            'f1': float(round(f1, 3)),
            'auc': float(round(auc_score, 3)),
    ...
    AUC, ACC, F1 = [], [], []
    for uid in results:
        if math.isnan(results[uid]["auc"]):
            continue
        ACC.append(results[uid]["acc"])
        AUC.append(results[uid]["auc"])
        F1.append(results[uid]["f1"])
        
    results["all"] = {
        'acc': round(sum(ACC) / len(ACC), 3),
        'f1': round(sum(F1) / len(F1), 3),
        'auc': round(sum(AUC) / len(AUC), 3)
    }
    print(results)

    now = datetime.datetime.now()
    output_filename = f'results/binary/fnn/{args.profile}-' + now.strftime('%Y%m%d_%H%M%S') + '.yaml'
