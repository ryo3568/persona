...
from model import BiLSTMEncoder, SelfAttentionMultiClassifier
...
def train_or_eval_model(model, loss_function1, loss_function2, dataloader, epoch, optimizer=None, train=False, rate=1.0):
    ...
    for data in dataloader:
        ...
        out = encoder(data)
        pred_persona, pred_sentiment = estimator(out)
...
if __name__ == '__main__':
    ...
    for i in range(args.iter):
        ...
        for testfile in tqdm(testfiles, position=0, leave=True):
            ...
            estimator = SelfAttentionMultiClassifier(D_h, D_h, D_o).cuda()
