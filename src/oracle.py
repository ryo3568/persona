...
def evaluation(a_scaler, v_scaler, test_id):
    for id in test_id:
        ...
        oracle = {0: [0, 1, 2], 1: [0, 1, 2], 2: [1, 2, 2], 3: [0, 1, 0]}

        profile = profiling(3, id)
        ...
        t_model = UnimodalFNN(input_dim=768, num_classes=num_classes).to(device)
        # t_file = glob.glob(f"model/{'ss' if args.ss else 'ts'}-{_t[0]}-{_t[1]}-t-*")[0]
        t_file = glob.glob(f"model/{'ss' if args.ss else 'ts'}-{0}-{0}-t-*")[0]
        t_model.load_state_dict(torch.load(t_file))

        a_model = UnimodalFNN(input_dim=384, num_classes=num_classes).to(device)
        a_file = glob.glob(f"model/{'ss' if args.ss else 'ts'}-{1}-{profiling(1 ,id)}-a-*")[0]
        a_model.load_state_dict(torch.load(a_file))

        v_model = UnimodalFNN(input_dim=66, num_classes=num_classes).to(device)
        v_file = glob.glob(f"model/{'ss' if args.ss else 'ts'}-{2}-{profiling(2, id)}-v-*")[0]
        v_model.load_state_dict(torch.load(v_file))
        ...
        with torch.no_grad():
            for t_x, a_x, v_x, y in test_loader:
                ...
                t_output = t_model(t_x)
                a_output = a_model(a_x)
                v_output = v_model(v_x)
                # outputの蓄積
                _t_output = torch.argmax(t_output, dim=1)
                _a_output = torch.argmax(a_output, dim=1)
                _v_output = torch.argmax(v_output, dim=1)
                t_outputs.extend(_t_output.tolist())
                a_outputs.extend(_a_output.tolist())
                v_outputs.extend(_v_output.tolist())
                # labelの蓄積
                labels.extend(y.tolist())
            for i in range(len(labels)):
                ...
                if 't' in args.modal and label == t_o:
                    t_flag = True 
                else:
                    t_flag = False
                if 'a' in args.modal and label == a_o:
                    a_flag = True 
                else:
                    a_flag = False
                if 'v' in args.modal and label == v_o:
                    v_flag = True
                else:
                    v_flag = False

                if t_flag or a_flag or v_flag:
...
def main(pgroup=0):
    train_id, _, test_id = get_ids(pgroup)
    ...
    evaluation(a_scaler, v_scaler, test_id)
...
if __name__ == '__main__':
    ...
    t_flag, a_flag, v_flag = False, False, False
