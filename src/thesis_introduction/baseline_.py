...
if __name__ == '__main__':
    ...
    for test_user in tqdm(users):
        ...
        1. データの準備
        '''
        x_train, y_train, x_test, y_test, x_fine, y_fine = load_data(test_user, args.modal)
        ...
        '''
        es = EarlyStopping(patience=5)
        x_train, x_valid, y_train, y_valid = train_test_split(x_fine, y_fine)
        for epoch in range(epochs):
            train_loss = 0.
            x_, y_ = shuffle(x_train, y_train)
            x_ = torch.Tensor(x_).to(device)
    ...
    print("personalised: ", round(sum(results) / len(results), 3))
