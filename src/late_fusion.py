...
from dataset import HazumiDataset
from model import LatefusionFNN
...
def evaluation(model, id, criterion, a_stds, v_stds):
    test_dataset = HazumiDataset(version=hazumi_version, ids=[id], ss=args.ss, a_stds=a_stds, v_stds=v_stds, test=True)
...
def main():
    ...
    for test_id in ids:
        ...
        train_dataset = HazumiDataset(version=hazumi_version, ids=train_id, ss=args.ss)
        valid_dataset = HazumiDataset(version=hazumi_version, ids=valid_id, ss=args.ss, a_stds=train_dataset.a_stds, v_stds=train_dataset.v_stds, test=True)
        train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True)
        valid_loader = torch.utils.data.DataLoader(dataset=valid_dataset, batch_size=batch_size, shuffle=False)

        model = LatefusionFNN(output_dim=output_dim, modal=args.modal).to(device)
        ...
        evaluation(model, test_id, criterion, a_stds=train_dataset.a_stds, v_stds=train_dataset.v_stds)
