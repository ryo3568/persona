...
from dataset import HazumiDataset_better, HazumiTestDataset_better
from model import UnimodalFNN, LatefusionFNN_better
...
def evaluation(model, a_scaler, v_scaler, test_id):
    for id in test_id:
        test_dataset = HazumiTestDataset_better(version=test_version, id=id, a_scaler=a_scaler, v_scaler=v_scaler, ss=args.ss)
...
def main(pgroup=0):
    ...
    train_dataset = HazumiDataset_better(version=train_version, ids=train_id, ss=args.ss)
    a_scaler, v_scaler = train_dataset.get_sscaler()
    valid_dataset = HazumiDataset_better(version=train_version, ids=valid_id, a_scaler=a_scaler, v_scaler=v_scaler, ss=args.ss)
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True)
    valid_loader = torch.utils.data.DataLoader(dataset=valid_dataset, batch_size=batch_size, shuffle=False)


    model = LatefusionFNN_better(input_dim=input_dim, num_classes=num_classes, ss=args.ss, pmode=args.pmode, pgroup=pgroup, modal=args.modal).to(device)
