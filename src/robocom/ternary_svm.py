...
import sys 
sys.path.append("../")
from utils import profiling, torch_fix_seed
from utils.EarlyStopping import EarlyStopping

def load_data(train, test, profile_vid):
    path = f'../../data/Hazumi_features/Hazumiall_features_ternary.pkl'
...
if __name__ == '__main__':
    ...
    for profile_n in range(profile_num):
        ...
        print(len(profile_vid))

        for train, test in kf.split(profile_vid):
            '''
            1. データの準備
            '''
            x_train, y_train, x_test, y_test = load_data(train, test, profile_vid)
